...
<body>
    ...
    <div id="cuerpo">
        <div id="cuerpoDatos" class="titulo-datos">
            <h2 class="titulo-princip margen-titulo">Datos de
        ...
        <?php
            ...
            $resultado->bindValue(":nombreYaci", $_SESSION['yacAc']);
            $resultado->execute();
            $rolRes;
            while($busqueda= $resultado->fetch(PDO::FETCH_ASSOC)){
            ...
            if(strcmp($rolRes,'admin')===0){
                header('Location:datosPrAdmin.php?nombreY='.$_SESSION['yacAc'].'&estado='.$_SESSION['estado']);
            }else{
 ...
 ?>
        <div>
            ...
            <button onclick="window.location='../menu.php'" class="btn btn-naranja btn-margenes">Volver a menú</button>
        </div>
    </div>
    </div>
    <footer>
